...
namespace DynamicGeometry
{
    ...
    public class BrushSerializer : ISerializer
    {
        ...
        public void Read(IValueProvider value, XElement serialized)
        {
            // Legacy drawings stored brushes as WPF XAML fragments; parse the
            // brush types that could actually occur by hand instead of a XAML reader.
        ...
        static Brush ParseBrush(XElement element)
        {
            switch (element.Name.LocalName)
            {
                case "SolidColorBrush":
                    return new SolidColorBrush(ReadColor(element.Attribute("Color")?.Value, Colors.Black));

                case "LinearGradientBrush":
                    var brush = new LinearGradientBrush();
                    brush.StartPoint = ReadRelativePoint(element.Attribute("StartPoint")?.Value, 0, 0);
                    brush.EndPoint = ReadRelativePoint(element.Attribute("EndPoint")?.Value, 1, 1);
                    foreach (var stop in element.Descendants().Where(e => e.Name.LocalName == "GradientStop"))
                    {
                        brush.GradientStops.Add(new GradientStop
                        {
                            Color = ReadColor(stop.Attribute("Color")?.Value, Colors.Black),
                            Offset = double.Parse(stop.Attribute("Offset")?.Value ?? "0", System.Globalization.CultureInfo.InvariantCulture)
                        });
                    }
                    return brush;
        ...
        static Color ReadColor(string text, Color fallback)
        {
            return string.IsNullOrEmpty(text) ? fallback : text.ToColor();
        }

        static Avalonia.RelativePoint ReadRelativePoint(string text, double defaultX, double defaultY)
        {
            if (!string.IsNullOrEmpty(text))
            {
                ...
                {
                    return new Avalonia.RelativePoint(x, y, Avalonia.RelativeUnit.Relative);
                }
            }

            return new Avalonia.RelativePoint(defaultX, defaultY, Avalonia.RelativeUnit.Relative);
        }
