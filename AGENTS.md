...
- **Gallery drawings** are embedded `.lgf` (`Gallery/Drawings`, order and titles in
  `GalleryCatalog`), generated by `tools/gallerize.cs` from the Windows Phone originals (outside
  the repo; never edit those). Change texts in the tool and rerun it rather than editing the
  output. Each has two unclickable labels, `Title` and `Description` (may contain live `[AB^2]`
